...
y_train = torch.from_numpy(
    y_train
).float()  # Ensure tensor is float32 for compatibility with loss computation
...
weight.requires_grad_()  # Enable gradient computation for weight parameter
bias = torch.zeros(
    1, requires_grad=True
)  # Set requires_grad directly in constructor instead of calling method


def model_fn(xb):
    return xb @ weight + bias  # Linear model: W*X + b


def loss_fn(input, target):
    return (input - target).pow(2).mean()  # Mean Squared Error loss
...
for epoch in range(num_epochs):
    for X_batch, y_batch in train_dl:
        ...
        loss = loss_fn(
            pred, y_batch.long()
        )  # Convert to long for cross-entropy compatibility (though MSE expects float)
    ...
    with torch.no_grad():
        ...
        weight.grad.zero_()  # Clear gradients for next epoch
        bias.grad.zero_()
